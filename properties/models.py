...
class RentalContract(models.Model):
    property = models.ForeignKey(Property, on_delete=models.CASCADE)  # العقار المؤجر
    tenant = models.ForeignKey(Tenant, on_delete=models.CASCADE)  # المستأجر
    start_date = models.DateField()  # تاريخ بدء الإيجار
    end_date = models.DateField()  # تاريخ انتهاء الإيجار

    def __str__(self):
        return f'Rental Contract: {self.property} - {self.tenant}'  # إعادة العقار والمستأجر عند تحويل الكائن إلى نص
